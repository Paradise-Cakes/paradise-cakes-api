...
resource "aws_route53_record" "dev_api_record" {
  zone_id = data.aws_route53_zone.paradise_cakes.zone_id
  name    = "dev-api.paradisecakesbymegan.com"
  type    = "A"

  alias {
    name                   = aws_api_gateway_domain_name.dev_api.cloudfront_domain_name
    zone_id                = aws_api_gateway_domain_name.dev_api.cloudfront_zone_id
    evaluate_target_health = true
  }
}
